				      ...
				      bool early)
{
	...
	if (early) {
		/*
		 * We can't use p4d_page_vaddr here as it would return a linear
		 * mapping address but it is not mapped yet, but when populating
		 * early_pg_dir, we need the physical address and when populating
		 * swapper_pg_dir, we need the kernel virtual address so use
		 * pt_ops facility.
		 */
		base_pud = pt_ops.get_pud_virt(pfn_to_phys(_p4d_pfn(*p4d)));
	 ...
	 */
	if (!early)
		set_p4d(p4d, pfn_p4d(PFN_DOWN(__pa(base_pud)), PAGE_TABLE));
}
				      ...
				      bool early)
{
	...
	do {
		...
		kasan_populate_pud(p4dp, vaddr, next, early);
...
#define kasan_populate_pgd_next(pgdp, vaddr, next, early)			\
		...
		(pgtable_l4_enabled ?						\
			kasan_populate_pud((p4d_t *)pgdp, vaddr, next, early) :	\
...
#define kasan_shallow_populate_pgd_next(pgdp, vaddr, next, kasan_populate)	\
		(pgtable_l5_enabled ?						\
		kasan_shallow_populate_p4d(pgdp, vaddr, next, kasan_populate) :	\
		kasan_shallow_populate_pud((p4d_t *)pgdp, vaddr, next, kasan_populate))
					      ...
					      bool kasan_populate)
{
	...
	base_pud = (pud_t *)p4d_page_vaddr(*p4dp);
	pudp = base_pud + pud_index(vaddr);
					      ...
					      bool kasan_populate)
{
	...
	if (kasan_populate)
		memcpy(base_p4d, (void *)kasan_early_shadow_pgd_next,
		       sizeof(p4d_t) * PTRS_PER_PUD);
	...
	do {
		...
		kasan_shallow_populate_pud(p4dp, vaddr, next, is_kasan_pud);
	} while (p4dp++, vaddr = next, vaddr != end);
}
...
static void __init kasan_shallow_populate_pgd(unsigned long vaddr, unsigned long end)
{
	...
	do {
		...
		kasan_shallow_populate_pgd_next(pgd_k, vaddr, next, is_kasan_pgd_next);
